...
function shellPath(value) {
  return decodeURI(
    new URL(value || "/root", "file://" + shell.cwd.replace(/\/?$/, "/"))
      .pathname,
  );
}
...
function kldstat() {
  return ["Id Refs Address            Size     Name"]
    .concat(
      Array.from(shell.modules, function (module, index) {
        return (
          ...
          (module === "kernel"
            ? "01d00000"
            : "000" + ((index + 4) * 16).toString(16).padStart(5, "0")) +
          " " +
          module
        );
      }),
...
function loadManuals() {
  return (
    shell.manuals ||
    (shell.manuals = fetch(shell.manualUrl)
...
function manualDescription(name, page) {
  ...
  return (
    ...
    page.section +
    ")" +
    (description ? " — " + description[1].trim().replace(/\s+/g, " ") : "")
  );
}
      ...
      .then(function (data) {
        return names
          .map(function (requested) {
            var name =
              data.aliases[requested.toLowerCase()] || requested.toLowerCase();
...
function ddbCommand(source) {
  ...
  switch (command) {
    ...
    case "alltrace":
      return (
        "Tracing command kradkrnl pid 404 tid " +
        ((ghostTick || 0) + 100404) +
        "\n" +
    ...
    case "show":
      ...
      if (argument === "locks" || argument === "alllocks") {
        return (
          "exclusive sleep mutex maze0-route (kradkrnl) r = 0 " +
          "(0x" +
          hex64(68) +
          ") locked @ krad_route_fault+0x404"
        );
      }
      if (argument === "thread") {
        return (
          "Thread 404/100404: kradkrnl.ko at maze0 cell " +
          position.x +
          "," +
          position.y
        );
      ...
      if (argument === "sysregs") {
        return (
          "cr0=0x80050039 cr2=0x" + hex64(69) + " cr3=0x00030000 cr4=0x00000000"
        );
      }
...
async function koalactlMove(pattern) {
  ...
  async function recurse(seq, depth) {
    ...
    lines.push(pad + "koalactl: cons('" + direction + "', \"" + rest + '")');
  ...
  switch (command) {
    ...
    case "uname":
      ...
      var uname = {
        ...
        s: "FreeBSD",
        U: "1402000",
        v: "FreeBSD 14.2-RELEASE-p4 KOALA",
      };
    ...
    case "id":
      if (!parts.length)
        return "uid=0(rad) gid=0(wheel) groups=0(wheel),5(operator)";
    ...
    case "uptime":
      return (
        ...
        Math.max(
          1,
          Math.floor((performance.now() - telemetry.started) / 60000),
        ) +
    ...
    case "top":
      return (
        "last pid: 528; load averages: 4.04, 1.19, 0.44\n" +
        "PID USERNAME PRI NICE SIZE STATE C TIME WCPU COMMAND\n" +
        "404 rad       404   -4  528K CPU" +
        (position.x % 4) +
        "  " +
        (turn || 0) +
        " 0:04 40.4% kradkrnl.ko"
      );
    case "kldstat":
      return kldstat();
    case "kldload":
      ...
      if (!names.length || /[^nqv]/.test(flags))
        return "usage: kldload [-nqv] file ...";
      return names
        .map(function (name) {
    ...
    case "procstat":
      return (
        "  PID  PPID  PGID   SID  TSID THR LOGIN WCHAN  EMUL COMMAND\n" +
    ...
    case "ifconfig":
      return (
        "koala0: flags=8843<UP,BROADCAST,RUNNING,SIMPLEX,MULTICAST>\n" +
    ...
    case "df":
      return (
        ...
        "maze0           4040 " +
        (pelletStarts.length - (pellets ? pellets.size : 0)) +
        "  4040     4% /recovery/route"
      );
    case "fsck":
      return (
      ...
      return names
        .map(function (target) {
          ...
          if (flags.includes("a") || flags.includes("A")) {
            entries = (flags.includes("a") ? [".", ".."] : []).concat(
              target === "/root" ? [".history"] : [],
              entries,
            );
          }
          ...
          if (flags.includes("l")) {
            return (
              ...
              entries
                .map(function (entry) {
                  return (
                    (target === "/" ? "drwxr-xr-x" : "-rw-r--r--") +
                    "  1 root wheel 404 Jul 30 04:04 " +
                    entry
                  );
                })
                .join("\n")
    ...
    case "cat":
      ...
      if (!names.length)
        return "cat: standard input is unavailable in recovery";
      return names
        .map(function (name) {
          name = shellPath(name);
          var content =
    ...
    case "koalactl":
      if (!argument || argument === "status") {
        return (
          "maze0: " +
          (running ? (paused ? "PAUSED" : "RUNNING") : "HALTED") +
          " cell=" +
          position.x +
          "," +
          position.y +
...
function appendShell(source, result, prompt) {
  ...
  shell.lines.forEach(function (line, index) {
    ...
    else if (/^[A-Z0-9_.+-]+\([1-9]\)/.test(value))
      span.className = "shell-man-title";
    else if (/^[A-Z][A-Z ]+$/.test(value)) span.className = "shell-man-heading";
    else if (
      /(?:usage:|not found|No manual entry|unknown|can't|unavailable|disabled)/i.test(
        value,
      )
    ) {
  ...
  shell.output.parentElement.scrollTop =
    shell.output.parentElement.scrollHeight;
}
...
shell.input.addEventListener("keydown", function (event) {
  ...
  if (event.code === "ArrowUp" || event.code === "ArrowDown") {
    ...
    shell.input.setSelectionRange(
      shell.input.value.length,
      shell.input.value.length,
    );
    return;
  }
